...
pub fn rope<T>(y: &mut Tensor<T>, start_pos: usize, theta: f32) 
where
    T: Float + Copy + Clone + Default,
{
    let shape = y.shape();
    ...
    for tok in 0..seq_len {
        ...
        for head in 0..n_heads {
            for i in 0..d / 2 {
                ...
                let freq = T::from(pos).unwrap() / T::from(theta.powf((i * 2) as f32 / d as f32)).unwrap();
    ...
    T: Float + Copy + Clone + Default,
{
    ...
    for b in 0..batch {
        ...
        for i in 0..seq_len {
            ...
            let mut sum = T::zero(); // 假设 T 是一个支持零值的类型，比如 f32 或 f64
            for j in 0..boundary {
    ...
    T: Float + Copy + Clone + Default,
{
    ...
    for i in 0..ext_loop {
        ...
        xp = (xp / T::from(inner_size).unwrap() + T::from(epsilon).unwrap()).sqrt();
        for j in 0..shape[last_dim] {
    ...
    T: Float + Copy + Clone + Default,
{
    ...
    for x in 0..a_rows {
        for y in 0..b_rows {
            let mut sum = T::zero();
            for k in 0..inner {
                let av: T = _a[x * inner + k];
                let bv: T = _b[y * inner + k];
                sum = sum + (av * bv);
            }
            _c[x * b_rows + y] = _c[x * b_rows + y] + (alpha * sum);
        }
    }
}
    ...
    T: Float + Copy + Clone + Default,
{
    ...
    impl<T: Clone + Float> From<(usize, &T)> for Probability {
        #[inline]
        fn from((i, p): (usize, &T)) -> Self {
            Self {
                val: p.to_f32().unwrap().clone(),
                tok: i as _,
            }
        }
    }
...
fn test_rms_norm_3() {
    let mut y = Tensor::<f32>::new(vec![
        1.1663, -1.3681, 0.3327, 2.0732,
        0.2235, -1.8537, -0.5217, -0.3280,
        -1.7655, 0.3192, 1.6616, 1.0265,
        -0.3022, 0.3825, -0.9588, -0.3537,
        0.0203, 0.2032, 1.0121, 0.2214,
        1.2528, -0.9799, -0.7158, -1.4957,
    ], &vec![2, 3, 2, 2]);
    let x = Tensor::<f32>::new(vec![
        1.1663, -1.3681, 0.3327, 2.0732,
        0.2235, -1.8537, -0.5217, -0.3280,
    ...
    assert!(y.close_to(
        &Tensor::<f32>::new(
            vec![
                0.9175, -2.1524, 0.2241, 2.7927,
                0.1693, -2.8081, -1.1972, -1.5056,
                -1.3916, 0.5033, 1.2031, 1.4866,
                -0.8767, 2.2193, -1.3268, -0.9790,
                0.1407, 2.8143, 1.3815, 0.6044,
                1.1139, -1.7426, -0.6105, -2.5513,
            ],
            &vec![2, 3, 2, 2]
        ),
        1e-3
